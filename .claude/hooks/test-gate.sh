...
run_integration_tests() {
    ...
    if [ -d "tests/integration" ] || [ -d "integration_tests" ]; then
        ...
        if [ -d "tests/integration" ]; then
            python -m pytest tests/integration --tb=short -q 2>&1 || test_result=$?
        else
            python -m pytest integration_tests --tb=short -q 2>&1 || test_result=$?
        fi
        return $test_result
    fi
    ...
    if [ -f "pytest.ini" ] || [ -f "pyproject.toml" ]; then
        if grep -q "integration" pytest.ini 2>/dev/null || grep -q "integration" pyproject.toml 2>/dev/null; then
            ...
            python -m pytest -m integration --tb=short -q 2>&1 || test_result=$?
            return $test_result
        fi
    fi
...
run_smoke_tests() {
    ...
    if [ -d "tests/smoke" ] || [ -d "smoke_tests" ]; then
        ...
        if [ -d "tests/smoke" ]; then
            python -m pytest tests/smoke --tb=short -q 2>&1 || test_result=$?
        else
            python -m pytest smoke_tests --tb=short -q 2>&1 || test_result=$?
        fi
        return $test_result
    fi
    ...
    if [ -f "pytest.ini" ] || [ -f "pyproject.toml" ]; then
        if grep -q "smoke" pytest.ini 2>/dev/null || grep -q "smoke" pyproject.toml 2>/dev/null; then
            ...
            python -m pytest -m smoke --tb=short -q 2>&1 || test_result=$?
            return $test_result
        fi
    fi
